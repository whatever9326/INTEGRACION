    ...
    reader.onload = async (e) => {
      if (isExcel) {
        ...
        rows.forEach((row, index) => {
          if (index < 2) return; // saltar encabezado y metadatos
          const model = String(row[1] || '').toLowerCase().replaceAll(' ', '');
          if (!model || model.includes('total')) return;

          console.log(`\n📦 Fila ${index + 1}: Modelo detectado: "${model}"`);

          colors.forEach((color, idx) => {
            const qty = row[idx + 2]; // desde columna 2 en adelante están los colores
            ...
            if (foundSku) {
              ...
              console.warn(`❌ No se encontró SKU para modelo: "${model}" + color: "${color}" con cantidad: ${qty}`);
            }
          });
        });
